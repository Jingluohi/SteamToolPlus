// 游戏数据管理服务
// 管理已下载和导入的游戏数据，存储在 PublicOut/config/game.json
...
/// 获取游戏数据文件路径
fn get_game_data_path(_app: &AppHandle) -> Result<std::path::PathBuf, String> {
    // 获取程序所在目录（exe文件所在的目录）
    ...
    // 直接在程序所在目录下创建 config/game.json
    // 不使用 PublicOut 子目录
    let data_path = exe_dir.join("config").join("game.json");
    
    Ok(data_path)
}

/// 确保数据目录存在
async fn ensure_data_dir_exists(app: &AppHandle) -> Result<(), String> {
    let data_path = get_game_data_path(app)?;
    let config_dir = data_path.parent().ok_or("无法获取配置目录")?;
...
/// 加载游戏数据
pub async fn load_game_data(app: AppHandle) -> Result<GameDataCollection, String> {
    let data_path = get_game_data_path(&app)?;
...
/// 保存游戏数据
pub async fn save_game_data(app: AppHandle, collection: &GameDataCollection) -> Result<(), String> {
    ensure_data_dir_exists(&app).await?;
