<template>
  <div class="vdf-to-lua-page">
    <div class="page-header">
      <h1>📄 VDF to Lua Converter</h1>
      <p class="subtitle">将 VDF 文件批量转换为 Steam 格式的 Lua 文件</p>
    </div>
              ...
              <pre>"depots"
{
    "depot_id"
    {
        "DecryptionKey" "key"
            ...
            <div class="format-block">
              <p><strong>生成的Lua格式：</strong></p>
              <code>addappid(depot_id, 0, "decryption_key")</code>
              <p class="example">示例：addappid(123456, 0, "a1b2c3d4e5f6...")</p>
            </div>

            <div class="format-block">
              <p><strong>转换规则：</strong></p>
              <ul>
                <li>主 App ID = 第一个 Depot ID - 1</li>
                <li>自动从同目录 .manifest 文件提取清单ID</li>
                <li>生成的 Lua 文件保存在 VDF 同目录</li>
              </ul>
            </div>
          </div>
        </div>
...
async function processFiles(files: File[]) {
  ...
  for (const file of files) {
    if (!file.name.toLowerCase().endsWith('.vdf')) continue
  ...
  if (addedCount > 0) {
    addLog(`✓ 成功添加 ${addedCount} 个 VDF 文件`, 'success')
  }
}
...
async function addFiles() {
  const selected = await open({
    multiple: true,
    filters: [{ name: 'VDF文件', extensions: ['vdf'] }]
  })
...
async function addFolder() {
  ...
  if (folder) {
    try {
      const files = await invoke<string[]>('get_vdf_files_in_folder', { folder })
      ...
      addLog(`✓ 从文件夹添加 ${files.length} 个 VDF 文件`, 'success')
    } catch (e) {
...
onMounted(async () => {
  unlisten = await listen<string[]>('vdf-files-dropped', (event) => {
    const files = event.payload
    addLog(`检测到 ${files.length} 个VDF文件被拖入`, 'info')
...
async function startConvert() {
  ...
  for (let i = 0; i < fileList.value.length; i++) {
    ...
    try {
      const result = await invoke<{ success: boolean; message: string; depotCount: number; mainAppId?: number }>(
        'convert_vdf_to_lua',
        { filePath: file.path }
      )

      if (result.success) {
        addLog(`    ✓ 成功 (${result.depotCount} 个 depot, 主AppID: ${result.mainAppId})`, 'success')
        file.status = 'success'
        stats.value.success++
      } else {
